...
class CMPC(Scene):
    def construct(self):
        ...
        falling_dots = [y.animate.set_value(new_func(x.get_value())) for x,y in zip(xts,yts)]
        ...
        new_graph = ax.plot(new_func, x_range=[-4, 4], color=BLUE)
        ...
        def updater_2(mobj, idx):
            mobj.add_updater(lambda x: x.move_to(ax.c2p(xts[idx].get_value(), new_func(xts[idx].get_value()))))
        ...
        xspace_right = np.linspace(-4, 4, 150)
        min_idx_left = new_func(xspace_left).argmin()
        min_idx_right = new_func(xspace_right).argmin()
